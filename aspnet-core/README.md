...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Backend.HttpApi.Host` and `Backend.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p cf06be12-1133-4d03-a4ae-58258e6e81a7
```

> `cf06be12-1133-4d03-a4ae-58258e6e81a7` is the password of the certificate, you can change it to any password you want.
...
Run `Backend.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Backend.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Backend.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
